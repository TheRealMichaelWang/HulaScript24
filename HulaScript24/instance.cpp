...
instance::instance(uint32_t max_locals, uint32_t max_globals, size_t max_table) : 
	...
	local_offset(0), extended_local_offset(0), global_offset(0), table_offset(0), start_ip(0), next_function_id(0), next_table_id(0), top_level_local_offset(0),
...
error instance::type_error(vtype expected, vtype got, uint32_t ip, std::vector<uint32_t> stack_trace) {
	...
	return error(etype::UNEXPECTED_TYPE, ss.str(), ip, stack_trace);
}